...
interface ChatWindowProps {
  ...
  onToggleSidebar: () => void; // UI POLISH (mobile) — buka/tutup drawer conversation
}
...
export default function ChatWindow({ title, messages, onSend, sending, disabled, onToggleSidebar }: ChatWindowProps) {
  ...
  return (
    <div className="flex flex-col h-full flex-1 min-w-0 bg-white">
      ...
      <div className="border-b border-slate-200 px-4 py-3 flex items-center gap-3">
        {/* UI POLISH (mobile) — tombol buka drawer conversation, cuma tampil di layar kecil */}
        <button
          type="button"
          onClick={onToggleSidebar}
          className="md:hidden shrink-0 rounded-lg p-1.5 text-slate-500 hover:bg-slate-100"
